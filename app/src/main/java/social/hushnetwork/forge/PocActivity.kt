...
class PocActivity : Activity() {
    ...
    private fun openWalletConnectUri(uri: String) {
        ...
        if (MobileWalletHarness.enabled) {
            setStatus("Pairing with local mobile harness wallet...", false)
            MobileWalletHarness.pair(uri) { result ->
                runOnUiThread {
                    result
                        .onSuccess {
                            appendLog(it)
                            setStatus("Local mobile harness wallet paired. Waiting for session approval...", false)
                        }
                        .onFailure {
                            setStatus("Harness pairing failed: ${it.message ?: it.javaClass.simpleName}", true)
                        }
                }
            }
            return
